...
namespace OperationManagement
{
    public partial class frmReports : Form
    {
        ...
        private void btnTotalOperationsValues_Click(object sender, EventArgs e)
        {
            ...
            {
                sb.Append("SELECT SUM(ConvertedValue) FROM Operations ");
            }            
            if (fullName != "")
            {
                sb.Append("WHERE FullName like '%" + fullName + "%' ");
            ...
            try
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        MessageBox.Show("Total das taxas operações: " + totalTaxValues.ToString("0.##") + " (" + taxValue + "% de " + totalConvertedValues + ")");
                    }
                }                
            }
            catch (Exception ex)
